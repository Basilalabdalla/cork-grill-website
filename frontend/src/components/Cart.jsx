...
const CartContent = ({ onCheckout, onClose }) => {
  ...
  return (
    <>
      <div className="flex justify-between items-center mb-6"><h2 className="text-2xl font-bold">Your Order</h2>{onClose && <button onClick={onClose} className="text-2xl font-semibold">×</button>}</div>
      {cartItems.length === 0 ? <p className="text-gray-500">Your cart is empty.</p> : <>
        <div className="space-y-4 max-h-64 overflow-y-auto pr-2">{cartItems.map(item => (<div key={item._id} className="flex justify-between items-center text-sm"><div><p className="font-bold">{item.name}</p><p className="text-gray-600">€{item.price.toFixed(2)}</p></div><div className="flex items-center gap-2"><button onClick={() => decreaseQuantity(item._id)} className="bg-gray-200 text-gray-800 rounded-full w-6 h-6 font-bold">-</button><span>{item.qty}</span><button onClick={() => increaseQuantity(item._id)} className="bg-gray-200 text-gray-800 rounded-full w-6 h-6 font-bold">+</button><button onClick={() => removeFromCart(item._id)} className="text-red-500 hover:text-red-700 ml-2"><TrashIcon /></button></div></div>))}</div>
...
const Cart = () => {
  ...
  // This line now correctly gets ALL the values we need from the context.
  const { cartItems, cartKey, handleCheckout } = useCart(); // Assuming handleCheckout is also provided by context now
        ...
        <AnimatePresence>
          {totalItems > 0 && (
            <motion.button
              ...
              initial={{ scale: 0 }}
              // --- FIX #2: Use a simpler animation that works with the default transition ---
              animate={{ scale: [1.2, 1] }} 
              exit={{ scale: 0 }}
              transition={{ duration: 0.2 }} // Use a simple duration instead of spring
